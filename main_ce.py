...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        ...
        train_dataset, val_dataset, _2, _3, _4 = ds.isic_dataloaders_sample(opt, is_sample=False)
...
def set_model(opt):
    model = SupCEResNet(name=opt.model, num_classes=opt.cls_num)
...
def train(train_loader, model, criterion, optimizer, epoch, opt, writer, iter_num):
    ...
    for idx, (images, labels, _) in enumerate(train_loader):
        ...
        if (idx + 1) % opt.print_freq == 0:
            ...
            writer.add_scalar('train_loss', losses.val, iter_num)
            sys.stdout.flush()

    return losses.avg, top1.avg, writer
...
def main():
    ...
    writer = SummaryWriter(opt.tb_folder + '/log')
    iter_num = 0
    # training routine
    for epoch in range(1, opt.epochs + 1):
        ...
        # train for one epoch
        time1 = time.time()
        loss, train_acc, writer = train(train_loader, model, criterion, optimizer, epoch, opt, writer, iter_num)
        time2 = time.time()
