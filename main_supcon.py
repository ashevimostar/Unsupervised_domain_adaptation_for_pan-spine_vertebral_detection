...
from loss.OriSupConLoss import OriSupConLoss
from loss.ConLoss import ConLoss
...
#     criterion = SupConLoss(opt, temperature=opt.temp, neg_size=opt.neg_size)
...
def set_model(opt):
    model = FixedSupConResNet(name=opt.model, num_classes=opt.cls_num)
    criterion_co = OriSupConLoss(temperature=opt.temp)
